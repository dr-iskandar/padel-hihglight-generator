...
def draw_zones(frame, zones):
    for name, points in zones.items():
        ...
        anchor = tuple(pts[0])
        cv2.putText(frame, name, (int(anchor[0]) + 4, max(20, int(anchor[1]) + 20)),
...
def draw_court_polygon(frame, court_polygon):
    ...
    cv2.putText(frame, "PLAYABLE COURT", (int(pts[0][0]) + 6, max(22, int(pts[0][1]) - 8)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.52, COURT_COLOR, 2, cv2.LINE_AA)


def draw_roi(frame, roi_px):
    x1, y1, x2, y2 = roi_px
    cv2.rectangle(frame, (x1, y1), (x2, y2), ROI_COLOR, 1)
    cv2.putText(frame, "AI ROI", (x1 + 5, max(20, y1 + 20)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.52, ROI_COLOR, 1, cv2.LINE_AA)
...
def draw_view_split(frame, half_polygons):
    ...
    for side, poly in half_polygons.items():
        ...
        cv2.putText(frame, f"{side.upper()} AI VIEW", (int(center[0]) - 55, int(center[1])),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.40, VIEW_COLOR, 1, cv2.LINE_AA)
...
def map_result_to_full_frame(result, crop_rect, id_offset=0):
    ...
    ids = [int(track_id) + int(id_offset) for track_id in result.boxes.id.int().cpu().tolist()]
...
def run_view_inference(model, frame, crop_rect, half_polygon, frame_shape, tracker_name, device,
                       conf_threshold, iou_threshold, imgsz, max_det, id_offset):
    x1, y1, x2, y2 = crop_rect
    crop = frame[y1:y2, x1:x2]
    if crop.size == 0:
        return [], np.empty((0, 4)), np.empty((0, 17, 2)), np.empty((0, 17))
    result = model.track(crop, persist=True, tracker=tracker_name,
                         conf=float(conf_threshold), iou=float(iou_threshold),
                         imgsz=int(imgsz), max_det=int(max_det), device=device,
                         verbose=False)[0]
    ids, boxes, xy, kp_conf = map_result_to_full_frame(result, crop_rect, id_offset=id_offset)
    return filter_detections_to_polygon(ids, boxes, xy, kp_conf, half_polygon, frame_shape)


def combine_detection_parts(parts):
    ids, boxes_parts, xy_parts, conf_parts, sides = [], [], [], [], []
...
def state_color(status: ServeStatus):
    if status.state == "COOLDOWN": return (0, 255, 0)
    if status.state == "SWING": return (0, 165, 255)
    if status.state == "PREPARING": return (0, 255, 255)
    if status.state == "IN_ZONE": return (255, 220, 100)
    return (230, 230, 230)


def format_track_label(track_id: int, side: str, near_offset: int, far_offset: int) -> str:
    if side == "far": return f"F{max(0, int(track_id) - int(far_offset))}"
    if side == "near": return f"N{max(0, int(track_id) - int(near_offset))}"
    return f"T{track_id}"


def draw_skeleton(frame, keypoints_xy, keypoints_conf, min_conf=0.30, color=(0, 255, 255)):
    for a, b in SKELETON_EDGES:
        if a >= len(keypoints_xy) or b >= len(keypoints_xy): continue
        if keypoints_conf[a] < min_conf or keypoints_conf[b] < min_conf: continue
    ...
    for i, point in enumerate(keypoints_xy):
        if i >= len(keypoints_conf) or keypoints_conf[i] < min_conf: continue
        cv2.circle(frame, tuple(np.rint(point).astype(int)), 3, color, -1, cv2.LINE_AA)


def draw_player_overlay(frame, bbox, keypoints_xy, keypoints_conf, status: ServeStatus,
                        side: str, near_offset: int, far_offset: int, min_kp_conf: float):
    ...
    title = f"{track_label} | {status.zone or '-'} | {status.state}"
    ...
    label_w, label_h = max(tw, dw) + 12, th + dh + 18
...
def main():
    ...
    if not fps or fps <= 1 or fps > 240: fps = 30.0
    ok, frame = cap.read()
    ...
    frame_stride = max(1, int(tracking.get("frame_stride", 2)))
    ...
    if court_polygon:
        serve_zones = derive_service_zones(court_polygon,
    ...
    view_padding = float(tracking.get("view_padding", 0.06))
    view_rects = ({side: polygon_to_crop_rect(poly, frame.shape, padding=view_padding)
                   for side, poly in half_polygons.items()} if dual_view else None)
    ...
    serve_detector = ServeDetector(serve_zones, cfg.get("serve_detector", {}), fps,
                                   court_polygon=court_polygon)

    recorder_cfg = cfg.get("recorder", {})
    output_dir = str(cfg.get("output_dir", "clips"))
    portrait_cfg = cfg.get("portrait_output", {})
    portrait_enabled = bool(portrait_cfg.get("enabled", True))
    ...
    show_portrait_crop = bool(portrait_cfg.get("show_source_crop", True))
    frame_index, last_event_text, last_event_until = 0, "", 0.0
    fps_ema, t_prev = 0.0, time.perf_counter()
    last_boxes = np.empty((0, 4), dtype=np.float32)
    last_ids, last_sides, last_statuses = [], [], {}
    ...
    try:
        while True:
            ...
            if landscape_recorder is not None:
                landscape_recorder.push(raw_frame, frame_index)
            vis = frame.copy()
            pending_event, pending_event_side = None, "single"

            if frame_index % frame_stride == 0:
                if dual_view:
                    near_part = run_view_inference(near_model, frame, view_rects["near"], half_polygons["near"],
                        frame.shape, tracker_name, device,
                        tracking.get("near_conf", tracking.get("conf", 0.28)), tracking.get("iou", 0.45),
                        tracking.get("near_imgsz", 768), tracking.get("max_det_per_view", 6), near_offset)
                    far_part = run_view_inference(far_model, frame, view_rects["far"], half_polygons["far"],
                        frame.shape, tracker_name, device,
                        tracking.get("far_conf", 0.18), tracking.get("iou", 0.45),
                        tracking.get("far_imgsz", 1024), tracking.get("max_det_per_view", 6), far_offset)
                    ids, boxes, xy, kp_conf, sides = combine_detection_parts([("near", near_part), ("far", far_part)])
                else:
                    rx1, ry1, rx2, ry2 = roi_px
                    crop = frame[ry1:ry2, rx1:rx2]
                    result = near_model.track(crop, persist=True, tracker=tracker_name,
                        conf=float(tracking.get("conf", 0.30)), iou=float(tracking.get("iou", 0.45)),
                        imgsz=int(tracking.get("imgsz", 960)), max_det=int(tracking.get("max_det", 8)),
                ...
                active_ids, active_sides, statuses = [], [], {}
                for i in range(count):
                    track_id, side = int(ids[i]), sides[i]
                    active_ids.append(track_id); active_sides.append(side)
                    kp = np.concatenate([xy[i], kp_conf[i][:, None]], axis=1)
                    event = serve_detector.update(track_id=track_id, bbox_xyxy=boxes[i], keypoints=kp,
                                                  frame_shape=frame.shape, frame_index=frame_index)
                    statuses[track_id] = serve_detector.get_status(track_id)
                    if event is not None:
                        pending_event, pending_event_side = event, side
                serve_detector.prune(frame_index)
                last_boxes, last_ids = boxes[:count], active_ids
                last_xy, last_kp_conf = xy[:count], kp_conf[:count]
                last_sides, last_statuses = active_sides, statuses
            ...
            if pending_event is not None:
                track_label = format_track_label(pending_event.track_id, pending_event_side,
                                                 near_offset, far_offset).lower()
                landscape_path = landscape_recorder.trigger(frame_index, label=f"serve_{track_label}") if landscape_recorder else None
                portrait_path = portrait_recorder.trigger(frame_index, label=f"serve_{track_label}_portrait") if portrait_recorder else None
                display_path = portrait_path or landscape_path
                last_event_text = (f"SERVE | {track_label.upper()} | {pending_event.zone} | "
                                   f"{pending_event.confidence:.0%} | {display_path.name if display_path else '-'}")
                last_event_until = time.monotonic() + 2.5
                print(last_event_text)

            if preview:
                if not dual_view: draw_roi(vis, roi_px)
                draw_court_polygon(vis, court_polygon); draw_zones(vis, serve_zones)
                ...
                for i, track_id in enumerate(last_ids):
                    if i >= len(last_boxes) or i >= len(last_xy) or i >= len(last_kp_conf): break
                    status = last_statuses.get(track_id, serve_detector.get_status(track_id))
                    side = last_sides[i] if i < len(last_sides) else "single"
                    draw_player_overlay(vis, last_boxes[i], last_xy[i], last_kp_conf[i], status,
                                        side, near_offset, far_offset, min_kp_conf)
                if portrait_enabled and portrait_reframer is not None and show_portrait_crop:
                    portrait_reframer.draw_source_crop(vis)

            now = time.perf_counter(); inst = 1.0 / max(1e-6, now - t_prev); t_prev = now
            fps_ema = inst if fps_ema == 0 else 0.90 * fps_ema + 0.10 * inst

            if preview:
                ai_mode = "DUAL" if dual_view else "SINGLE"
                is_recording = ((landscape_recorder is not None and landscape_recorder.active) or
                                (portrait_recorder is not None and portrait_recorder.active))
                status_line = (f"LOOP {fps_ema:.1f} FPS | SRC {fps:.1f} | AI {ai_mode} /{frame_stride} | "
                               f"{'RECORDING CLIP' if is_recording else 'BUFFERING'}")
                cv2.putText(vis, status_line, (18, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.68,
                ...
                if portrait_preview and portrait_frame is not None:
                    ...
                    cv2.imshow("Portrait Output 9:16", cv2.resize(portrait_frame, (preview_w, preview_h), interpolation=cv2.INTER_AREA))
                key = cv2.waitKey(1) & 0xFF
                if key in (27, ord("q"), ord("Q")): break

            frame_index += 1
    finally:
        if landscape_recorder is not None: landscape_recorder.close()
        if portrait_recorder is not None: portrait_recorder.close()
